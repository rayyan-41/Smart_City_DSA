...
public:
    ...
    Point2D geoToCanvas(double lat, double lon) const {
        ...
        // Force a wider stance for better terminal visualization
        double widthStretch = 2.5;
    ...
    void zoomOut() { zoom /= 1.1; if (zoom < 0.5) zoom = 0.5; }
...
// Revamped Dijkstra Workflow
enum class DijkstraMode {
    START_SECTOR,       // Step 1: Filter Start by Sector
    START_CATEGORY,     // Step 2: Filter Start by Type
    START_NODE,         // Step 3: Pick Start Node

    DESTINATION_MODE,   // Step 4: Choose "Specific" or "Nearest"

    END_SECTOR,         // Step 5a: Filter End by Sector (if specific)
    END_CATEGORY,       // Step 5b: Filter End by Type (if specific)
    END_NODE,           // Step 5c: Pick End Node (if specific)

    RUNNING,            // Step 6: Calculating
    COMPLETE            // Step 7: Result
};
...
private:
    ...
    int dijkstraEndNode;
    string dijkstraTargetType;  // "SCHOOL", "HOSPITAL", etc.
    Vector<int> dijkstraPath;
    double dijkstraDistance;

    // UI Selection State
    int menuIndex; // Current selection index
    std::vector<string> menuOptions; // Text to display
    std::vector<int> menuNodeIDs; // IDs corresponding to options (for node selection)
...
public:
    ...
    void updateFilteredNodes(); // For visual feedback
...
// ============================================================================
// MAIN SIMULATION LOOP (THE MISSING PIECE)
// ============================================================================
...
inline Canvas CitySimulator::renderGraphToCanvas(int width, int height, bool onlyShowPath) {
    ...
    // Layer 1: Sector Bounds (Hide in Dijkstra Mode)
    ...
    if (showRoads) {
        for (const auto& edge : graphEdges) {
            // Dijkstra Filtering: Only show edges on the path
            ...
            if (idx1 >= 0 && idx2 >= 0) {
                ...
                else if (n1.isCorner && n2.isCorner) roadColor = Color::GrayDark;
                else roadColor = Color::GrayLight;

                if (onlyShowPath) {
                    // Block-style line for Dijkstra
    ...
    for (const auto& node : graphNodes) {
        // Dijkstra Filtering: Only show nodes on path, or Start/End
        if (onlyShowPath) {
        ...
        else if (node.isFiltered) c = Color::White; // Visual Feedback for Filters

        auto pen = stylize(c);

        // Special handling for Dijkstra Corners: Show them as distinct blocks
        ...
        if (node.type == "HOSPITAL") {
            ...
            canvas.DrawPointLine(x - 1, y + 1, x + 1, y + 1, pen);
        }
            ...
            canvas.DrawPoint(x, y, true, stylize(Color::DarkSlateGray2));
        }
        else {
            canvas.DrawPoint(x, y, true, pen);
    ...
    // Layer 4: Vehicles (Hide in Dijkstra Mode)
...
// Updates the menu options based on current stage
inline void CitySimulator::updateDijkstraMenu() {
    ...
    if (dijkstraMode == DijkstraMode::START_SECTOR || dijkstraMode == DijkstraMode::END_SECTOR) {
        ...
        // Filter nodes based on selected sector and category
...
// Visual Feedback: Highlight nodes matching current menu
inline void CitySimulator::updateFilteredNodes() {
    for (auto& node : graphNodes) node.isFiltered = false;

    // Only highlight in node selection phase
...
inline void CitySimulator::runDijkstraView() {
    ...
    auto renderer = Renderer([&] {
        ...
        // Pass 'true' to render only path nodes/edges + use Block rendering for path
        bool showPathOnly = (dijkstraMode == DijkstraMode::COMPLETE);
        Canvas c = renderGraphToCanvas(termW - 35, termH - 4, showPathOnly);
        ...
        // Selection Context (Breadcrumbs)
        if (dijkstraStartNode != -1) {
            int idx = nodeIdToIndex[dijkstraStartNode];
            items.push_back(hbox({ text("FROM: ") | dim, text(graphNodes[idx].name.substr(0,18)) | color(Color::Cyan) }));
        }
        ...
        if (dijkstraMode != DijkstraMode::COMPLETE) {
            ...
            if (dijkstraEndNode == -1) items.push_back(text("No Path Found!") | color(Color::Red));
                ...
                items.push_back(hbox({ text("TO:   "), text(graphNodes[eIdx].name.substr(0,18)) | color(Color::Yellow) }));
        ...
        auto panel = vbox(items) | border | size(WIDTH, EQUAL, 30);
        return vbox({
...
inline void CitySimulator::runGraphView() {
    ...
    auto renderer = Renderer([&] {
        ...
        auto infoPanel = vbox({
            ...
            hbox({text("x") | color(Color::Magenta), text(" Pharmacy")}),
            hbox({text("◆") | color(Color::Magenta), text(" Mall")}), // Malls Magenta
            hbox({text("⌂") | color(Color::BlueLight), text(" House")}), // Houses Blue
            separator(),
